...
#                                                                              #
# - Sets of 30 streams and changes the pattern of the first two bytes.         #  
...
struct_new streamIP StreamIP
        ...
        set streamIP(DestinationMAC.1.uc) [format %c 2]
        set streamIP(DestinationMAC.2.uc) [format %c 3]
        set streamIP(DestinationMAC.3.uc) [format %c 4]
        set streamIP(DestinationMAC.4.uc) [format %c 5]
        set streamIP(DestinationMAC.5.uc) [format %c 6]
	set streamIP(SourceMAC.0.uc) [format %c 9]
        set streamIP(SourceMAC.1.uc) [format %c 8]
        set streamIP(SourceMAC.2.uc) [format %c 7]
        set streamIP(SourceMAC.3.uc) [format %c 6]
        set streamIP(SourceMAC.4.uc) [format %c 5]
        set streamIP(SourceMAC.5.uc) [format %c 4]
        set streamIP(TimeToLive) [format %c 10]
        set streamIP(DestinationIP.0.uc) [format %c 0x00]
        set streamIP(DestinationIP.1.uc) [format %c 0x99]
        set streamIP(DestinationIP.2.uc) [format %c 0x88]
        set streamIP(DestinationIP.3.uc) [format %c 0x77]
        set streamIP(SourceIP.0.uc) [format %c 0x22]
        set streamIP(SourceIP.1.uc) [format %c 0x33]
        set streamIP(SourceIP.2.uc) [format %c 0x44]
        set streamIP(SourceIP.3.uc) [format %c 0x55]
...
struct_new incrementIP StreamIP
	...
	set incrementIP(SourceIP.1.uc) [format %c 1]
        set incrementIP(SourceIP.3.uc) [format %c 1]
        set incrementIP(DestinationIP.3.uc) [format %c 2]

#Set up multiple streams on the card
LIBCMD HTSetStructure $L3_DEFINE_MULTI_IP_STREAM $SOURCE_STREAM $NUM_STREAMS_ADDED 0 incrementIP 0 $iHub $iSlot $iPort

#Unset the structure
unset incrementIP

####################################################################################################
# Specify transmit parameter for the first stream                                                  #
####################################################################################################

struct_new ext L3StreamExtension

	set ext(ulFrameRate)  100
	set ext(ulTxMode)     $L3_SINGLE_BURST_MODE
	set ext(ulBurstCount) $BURST_PER_STREAM
	set ext(ulBurstGap)   96 
LIBCMD HTSetStructure $L3_DEFINE_STREAM_EXTENSION 0 0 0 ext 0 $iHub $iSlot $iPort
unset ext
...
####################################################################################################
struct_new incrementExt L3StreamExtension
	set incrementExt(ulFrameRate)  1
LIBCMD HTSetStructure $L3_DEFINE_MULTI_STREAM_EXTENSION $SOURCE_STREAM $NUM_STREAMS_ADDED 0 incrementExt 0 \
 $iHub $iSlot $iPort
unset incrementExt
