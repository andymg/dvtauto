  ...
  for {set i 0} {$i < $maxcards} {incr i} {    
	  ...
	  set txcount 0
       ...
       #######################################################################
        ...
        set streamIP(ucRandomLength) [format %c 0]
        ...
        set streamIP(DestinationMAC.3.uc) [format %c [expr $S2 + 1]]
        ...
        set streamIP(SourceMAC.3.uc) [format %c [expr $S + 1]]
	...
	for {set j 0} {$j < 4} {incr j} {
           ...
           if {$octet == "X" } {
              set streamIP(DestinationIP.$j.uc) [format %c [expr $S2 +1]]
           } else {
	      set streamIP(DestinationIP.$j.uc) [format %c $octet]
           }
	}
	# SOURCE IP
 	for {set k 0} {$k < 4} {incr k} {
           ...
           if { $octet == "X" } {
              set streamIP(SourceIP.$k.uc) [format %c [expr $S +1]]
	   } else {
	      set streamIP(SourceIP.$k.uc) [format %c $octet]
           }
	}
	# GATEWAY IP (ROUTER PORT)
	# You will need to uncomment and specify a per stream gateway if the
	# default gateway (set with cards L3 address) is insufficient
#	for {set l 0} {$l < 4} {incr l} {
#           set octet [lindex [split $gateway_ip .] $l]
#           if { $octet == "X" } {
#              set streamIP(Gateway.$l.uc) [format %c [expr $S + 1]]
#           } else {
#	   set streamIP(Gateway.$l.uc) [format %c $octet]
#           }           
#        }
	...
	set streamIP(TypeOfService) [format %c $TOS]
        ...
        if {$SEQUENCE < 0} {
            ...
            set incrementIP(DestinationIP.3.uc) [format %c 1]
...
proc set_streamipx { {txgroup} {rxgroup} {NUM_STREAMS 10} } {
  ...
  for {set i 0} {$i < $maxcards} {incr i} {    
	  ...
	  set txcount 0
       ...
       ###################### STREAM SETUP ###################################
       #######################################################################
        set streamIPX(ucActive) [format %c 1]
        set streamIPX(ucProtocolType) [format %c $STREAM_PROTOCOL_IPX]
	set streamIPX(ucRandomLength) [format %c 0]
	set streamIPX(ucRandomData) [format %c 0]
        set streamIPX(uiFrameLength) $DATA_LENGTH
        set streamIPX(ucTagField) [format %c 1]

        set streamIPX(DestinationMAC.0.uc) [format %c 0]
        set streamIPX(DestinationMAC.1.uc) [format %c 0]
        set streamIPX(DestinationMAC.2.uc) [format %c 0]
        set streamIPX(DestinationMAC.3.uc) [format %c [expr $S2 + 1]]
        set streamIPX(DestinationMAC.4.uc) [format %c 0]
        set streamIPX(DestinationMAC.5.uc) [format %c 1]
        set streamIPX(SourceMAC.0.uc) [format %c 0]
        set streamIPX(SourceMAC.1.uc) [format %c 0]
        set streamIPX(SourceMAC.2.uc) [format %c 0]
        set streamIPX(SourceMAC.3.uc) [format %c [expr $S + 1]]
        set streamIPX(SourceMAC.4.uc) [format %c 0]
        set streamIPX(SourceMAC.5.uc) [format %c 1]
        set streamIPX(IPXlen) 10
	set streamIPX(IPXhop) [format %c 10]
	set streamIPX(IPXtype) [format %c 0]
	# DESTINATION NET
	for {set j 0} {$j < 4} {incr j} {
           set octet [lindex [split $ipxdest_net] $j]
           if {$octet == "X" } {
              set streamIPX(IPXdst.$j.uc) [format %c [expr $S2 +1]]
           } else {
	      set streamIPX(IPXdst.$j.uc) [format %c $octet]
           }
	}
	# DESTINATION HOST
	for {set k 0} {$k < 6} {incr k} {
           ...
           if {$octet == "X" } {
              set streamIPX(IPXdstHost.$k.uc) [format %c [expr $S2 +1]]
           } else {
	      set streamIPX(IPXdstHost.$k.uc) [format %c $octet]
           }
	}
        # DESTNATION SOCKET
	...
	for {set l 0} {$l < 4} {incr l} {
           ...
           if {$octet == "X" } {
              set streamIPX(IPXsrc.$l.uc) [format %c [expr $S +1]]
           } else {
	      set streamIPX(IPXsrc.$l.uc) [format %c $octet]
           }
	}
	# SOURCE HOST
	for {set m 0} {$m < 6} {incr m} {
           ...
           if {$octet == "X" } {
              set streamIPX(IPXsrcHost.$m.uc) [format %c [expr $S +1]]
           } else {
	      set streamIPX(IPXsrcHost.$m.uc) [format %c $octet]
           }
	}
        # SOURCE SOCKET
	set streamIPX(IPXdstSocket) $ipxsource_socket
        ...
        if {$SEQUENCE < 0} {
            ...
            #    Last byte of Destination and Source IPs will increment by 1
            ############################################################
            set incrementIPX(DestinationMAC.5.uc) [format %c 1]
            set incrementIPX(SourceMAC.5.uc) [format %c 1]
	    set incrementIPX(IPXdstHost.5.uc) [format %c 1]
	    set incrementIPX(IPXsrcHost.5.uc) [format %c 1]
  ...
  for {set i 0} {$i < $maxcards} {incr i} {    
	  ...
	  set txcount 0
       ...
       #######################################################################
        ...
        set streamUDP(ucActive) [format %c 1]
        set streamUDP(ucProtocolType) [format %c $STREAM_PROTOCOL_UDP]
        set streamUDP(uiFrameLength) $DATA_LENGTH
        set streamUDP(ucTagField) [format %c 1]
        set streamUDP(DestinationMAC.0.uc) [format %c 0]
        set streamUDP(DestinationMAC.1.uc) [format %c 0]
        set streamUDP(DestinationMAC.2.uc) [format %c 0]
        set streamUDP(DestinationMAC.3.uc) [format %c [expr $S2 + 1]]
        set streamUDP(DestinationMAC.4.uc) [format %c 0]
        set streamUDP(DestinationMAC.5.uc) [format %c 1]
        set streamUDP(SourceMAC.0.uc) [format %c 0]
        set streamUDP(SourceMAC.1.uc) [format %c 0]
        set streamUDP(SourceMAC.2.uc) [format %c 0]
        set streamUDP(SourceMAC.3.uc) [format %c [expr $S + 1]]
        set streamUDP(SourceMAC.4.uc) [format %c 0]
        set streamUDP(SourceMAC.5.uc) [format %c 1]
        set streamUDP(TimeToLive) [format %c 10]
	# DESTINATION IP
	for {set j 0} {$j < 4} {incr j} {
           set octet [lindex [split $destination_ip .] $j]
           if {$octet == "X" } {
              set streamUDP(DestinationIP.$j.uc) [format %c [expr $S2 +1]]
           } else {
	      set streamUDP(DestinationIP.$j.uc) [format %c $octet]
           }
	}
	# SOURCE IP
 	for {set k 0} {$k < 4} {incr k} {
           set octet [lindex [split $source_ip .] $k]
           if { $octet == "X" } {
              set streamUDP(SourceIP.$k.uc) [format %c [expr $S +1]]
	   } else {
	      set streamUDP(SourceIP.$k.uc) [format %c $octet]
           }
	}
	# GATEWAY IP (ROUTER PORT)
	# You will need to uncomment and specify a per stream gateway if the
...
#           set octet [lindex [split $gateway_ip .] $l]
#           if { $octet == "X" } {
#              set streamUDP(Gateway.$l.uc) [format %c [expr $S + 1]]
#           } else {
#	   set streamUDP(Gateway.$l.uc) [format %c $octet]
#           }           
#        }
        set streamUDP(Netmask.0.uc) [format %c 255]
        set streamUDP(Netmask.1.uc) [format %c 255]
        set streamUDP(Netmask.2.uc) [format %c 255]
        set streamUDP(Netmask.3.uc) [format %c 0]
	set streamUDP(TypeOfService) [format %c $TOS]
	set streamUDP(UDPSrc) 	4369
 	...
 	set streamUDP(UDPLen) 100
        ...
        if {$SEQUENCE < 0} {
            ...
            #    Last byte of Destination and Source IPs will increment by 1
            ############################################################
            set incrementUDP(DestinationMAC.5.uc) [format %c 1]
            set incrementUDP(SourceMAC.5.uc) [format %c 1]
            set incrementUDP(DestinationIP.3.uc) [format %c 1]
            set incrementUDP(SourceIP.3.uc) [format %c 1]
...
foreach card $group {
   ...
   set P [lindex $card 2]
    ...
    if { $DefStreams(0.ul)  < 2 } {
       ...
       for {set j 1} {$j < $streams} {incr j} {
          ...
          for {set i 0} {$i < 6} {incr i} {
             puts  -nonewline $output " [format %02X  [C2I $SB(ProtocolHeader.$i.uc)]]"
          }
          puts $output ""
          ...
          switch [C2I $SB(ucProtocolType)] {
	  0 { puts $output "SmartBits"}
          2 { puts $output "IP"
              puts -nonewline $output "    Destination IP       ==> "
              for {set i 16} {$i < 19} {incr i} {
                 puts  -nonewline $output "[format %d  [C2I $SB(ProtocolHeader.$i.uc)]]."
              }
              puts $output "[format %d  [C2I $SB(ProtocolHeader.19.uc)]]"   

              puts -nonewline $output "    Source IP            ==> "
              for {set i 20} {$i < 23} {incr i} {
                 puts  -nonewline $output "[format %d  [C2I $SB(ProtocolHeader.$i.uc)]]."
              }
              puts $output "[format %d  [C2I $SB(ProtocolHeader.23.uc)]]"   
              puts $output "\n   Frame length is $SB(uiFrameLength) bytes"
	      }
	   3 { puts $output "IPX"
              puts -nonewline $output "    Dest Net ==> "
              for {set i 16} {$i < 20} {incr i} {
                 puts  -nonewline $output "[format %02d  [C2I $SB(ProtocolHeader.$i.uc)]] "
              }
              puts -nonewline $output " Dest Host ==> "
              for {set i 20} {$i < 26} {incr i} {
                 puts  -nonewline $output "[format %02d  [C2I $SB(ProtocolHeader.$i.uc)]] "
              }
	      puts -nonewline $output " Sckt => [format %02d [C2I $SB(ProtocolHeader.26.uc)]]"
              puts ""
	      ...
	      puts -nonewline $output " Sckt => [format %02d [C2I $SB(ProtocolHeader.38.uc)]]"
              puts $output ""
	     }
	  4 { puts $output "UDP"
             puts -nonewline $output "    Destination IP  ==> "
             ...
             puts "[format %d  [C2I $SB(ProtocolHeader.19.uc)]]"   
	...
	# Check ARP state - when ARP is sent time is recorded in 52 53 53 and 55
        ...
        set ARPStart 0
        set ARPStart [expr $ARPStart + [C2I $SB(ProtocolHeader.54.uc)] * 0x10000]
        set ARPStart [expr $ARPStart + [C2I $SB(ProtocolHeader.53.uc)] * 0x100]
        set ARPStart [expr $ARPStart + [C2I $SB(ProtocolHeader.52.uc)] * 1]

        set ARPEnd 0
        set ARPEnd [expr $ARPEnd + [C2I $SB(ProtocolHeader.58.uc)] * 0x10000]
        set ARPEnd [expr $ARPEnd + [C2I $SB(ProtocolHeader.57.uc)] * 0x100]
        set ARPEnd [expr $ARPEnd + [C2I $SB(ProtocolHeader.56.uc)] * 1]
...
proc show_l3 { {group} {output stdout} } {
   ...
   puts $output "==================================================="
   puts -nonewline $output " Card MAC Address 	==> [format "%02X" [C2I $L3Addr(szMACAddress.0.uc)]] "
   puts -nonewline $output "[format "%02X" [C2I $L3Addr(szMACAddress.1.uc)]] "
   puts -nonewline $output "[format "%02X" [C2I $L3Addr(szMACAddress.2.uc)]] "
   puts -nonewline $output "[format "%02X" [C2I $L3Addr(szMACAddress.3.uc)]] "
   puts -nonewline $output "[format "%02X" [C2I $L3Addr(szMACAddress.4.uc)]] "
   puts $output "[format "%02X" [C2I $L3Addr(szMACAddress.5.uc)]]"

   puts -nonewline $output " Card stack IP Address 	==> [C2I $L3Addr(IP.0.uc)]."
   puts -nonewline $output "[C2I $L3Addr(IP.1.uc)]."
   puts -nonewline $output "[C2I $L3Addr(IP.2.uc)]."
   puts $output "[C2I $L3Addr(IP.3.uc)]"

   puts $output "---------------------------------------------------"

   puts -nonewline $output " Netmask 		==> [C2I $L3Addr(Netmask.0.uc)]."
   puts -nonewline $output "[C2I $L3Addr(Netmask.1.uc)]."
   puts -nonewline $output "[C2I $L3Addr(Netmask.2.uc)]."
   puts $output "[C2I $L3Addr(Netmask.3.uc)]"

   puts -nonewline $output " Gateway IP 		==> [C2I $L3Addr(Gateway.0.uc)]."
   puts -nonewline $output "[C2I $L3Addr(Gateway.1.uc)]."
   puts -nonewline $output "[C2I $L3Addr(Gateway.2.uc)]."
   puts $output "[C2I $L3Addr(Gateway.3.uc)]"

   puts $output "---------------------------------------------------"

   puts -nonewline $output " Ping Target IP 	==> [C2I $L3Addr(PingTargetAddress.0.uc)]."
   puts -nonewline $output "[C2I $L3Addr(PingTargetAddress.1.uc)]."
   puts -nonewline $output "[C2I $L3Addr(PingTargetAddress.2.uc)]."
   puts $output "[C2I $L3Addr(PingTargetAddress.3.uc)]"
   puts $output ""
...
for {set i 0} {$i < 4} {incr i} {
  set addr(IP.$i.uc) [format %c [lindex [split $smartcard_ip .] $i]]
}
...
##########################################
for {set i 0} {$i < 4} {incr i} {
  set addr(Gateway.$i.uc) [format %c [lindex [split $gateway_ip .] $i]]
}
##########################################
#  Layer 3 Card Ping Target
###########################################
for {set i 0} {$i < 4} {incr i} {
  set addr(PingTargetAddress.$i.uc) [format %c [lindex [split $ping_target .] $i]]
}
###########################################
# Set Control values
###########################################
...
set addr(szMACAddress.5.uc) [format %c [expr $S + 1]]
##########################################
