...
namespace eval libx {
 ...
 namespace export run stop burst set_count show_count set_capture \
	 ...
	 check_link check_cardlink LIBCMD
###############################################################
...
proc set_link {} {
 ...
 if { [uplevel #0 info exists __SMARTLIB_TCL__] < 1 } {
    ...
    puts "You must source SmartLib.tcl before running SmartLib scripts"
   ...
   LIBCMD HTGetCardModel cardname $H $S $P
      switch $cardname {
	   ...
	   SX-7410  {
		   ...
		   set CapParams(ucCRCErrors)   0
		   set CapParams(ucOnTrigger)   0
		   set CapParams(ucFilterMode)  0
		   set CapParams(ucStartStopOnConditionXMode) 0
		   set CapParams(uc64BytesOnly) 0
		   set CapParams(ucLast64Bytes) 0
		   set CapParams(ucCollisions)  0
		   set CapParams(ucStartStop)    1
	   ...
	   LAN-6200A/3200A
		   ...
		   set CapSetup(ucCRCErrors)       0
		   set CapSetup(ucRxTrigger)       0
		   set CapSetup(ucTxTrigger)       0
		   set CapSetup(ucRCErrors)        0
		   set CapSetup(ucFilterMode)      0
		   set CapSetup(ucStartStopOnConditionMode)        0
		   set CapSetup(uc64BytesOnly)     0
		   set CapSetup(ucLast64Bytes)     0
		   set CapSetup(ucStartStop)        1
...
proc show_capture { {group} {number2Show 5} {output stdout} } {
      ...
      switch $cardname {
	   ...
	   SX-7410  {
		    ...
		    if {$CapCount(ulCaptureCount) < 1} {
		           ...
		           if {$CapCount(ulCaptureCount) < $number2Show} {
     		              set number2Show $CapCount(ulCaptureCount)
                           }
		 	   puts $output "Displaying $number2Show packets of $CapCount(ulCaptureCount) captured on card [expr $S + 1]"
			   struct_new CapInfo FSTCaptureInfo
			   LIBCMD HTGetStructure $::FST_CAPTURE_INFO 0 $number2Show 0 CapInfo 0 $H $S $P
		 	   struct_new CapData FSTCaptureDataInfo
		   	      for {set i 0} {$i < $number2Show} {incr i} {
	   ...
	   ML-7710 {
		    ...
		    LIBCMD HTGetStructure $::L3_CAPTURE_COUNT_INFO 0 0 0 CapCount 0 $H $S $P
		      if {$CapCount(ulCount) < 1} {
		           ...
		           if {$CapCount(ulCount) < $number2Show} {
     		             set number2Show $CapCount(ulCount)
                           }
		           puts $output "Displaying $number2Show packets of $CapCount(ulCount) captured on card [expr $S + 1]"
			...
			# format 02X prepends leading 0 _ubyte_ corrects for signed char                                
	   ...
	   LAN-6200A/3200A
		    {
	            ...
	            LIBCMD HTGetStructure $::GIG_STRUC_CAP_COUNT_INFO 0 0 0 CapCount 0 $H $S $P
	               ...
	               puts $output "Displaying $number2Show packets of $CapCount(ulCount) captured on card [expr $S + 1]"
                       ...
                       LIBCMD HTGetStructure $::GIG_STRUC_CAP_INFO 0 $number2Show 0 CapInfo 0 $H $S $P
...
foreach card $group {
   ...
   set P [lindex $card 2]
	   ...
	   ML-7710 {
                    ...
                    puts $output "***************************************************************"
                    puts $output ""
                    for {set register 0} {$register < $maxRegisters} {incr register} {
 ...
 check_link
  ...
  foreach card $group {
   ...
   #//////////////////////////////set_ethernet////////////////////////////
	...
	#######################################################
	LIBCMD HTDataLength $dataLength $H $S $P
	...
	HTFillPattern $dataLength filldata $H $S $P
	unset filldata
        }
   #///////////////////////////end set_ethernet///////////////////////////
   ...
   #////////////////////////////////set_gigabit/////////////////////////////
	...
	set GigTX(uiMainLength) $dataLength
	set GigTX(ucPreambleByteLength)  8
	set GigTX(ucFramesPerCarrier)   1
	set GigTX(ulGap) 96
	set GigTX(ucMainRandomBackground)  $::DISABLE
	set GigTX(ucBG1RandomBackground)  $::DISABLE
	set GigTX(ucBG2RandomBackground)  $::DISABLE
	set GigTX(ucSS1RandomBackground)  $::DISABLE
	set GigTX(ucSS2RandomBackground)  $::DISABLE
	set GigTX(ucMainCRCError)  $::DISABL
	set GigTX(ucBG1CRCError)  $::DISABLE
	set GigTX(ucBG2CRCError)  $::DISABLE
	set GigTX(ucSS1CRCError)  $::DISABLE
	set GigTX(ucSS2CRCError)  $::DISABLE
	set GigTX(ucJabberCount)  $::DISABLE
	set GigTX(ucLoopback)  $::DISABLE
	...
	set GigTX(ucVFD1Mode)  $::GIG_VFD_OFF
	set GigTX(ucVFD2Mode)  $::GIG_VFD_OFF
	set GigTX(ucVFD3Mode)  $::GIG_VFD3_OFF
	set GigTX(ucMainBG1Mode)  $::DISABLE
	...
	set GigTX(ucEchoMode)  $::DISABLE
	set GigTX(ucPeriodicGap) 0
	set GigTX(ucCountRcverrOrOvrsz) 0
	set GigTX(ucGapByBitTimesOrByRate) 0
	set GigTX(ucRandomLengthEnable)  $::DISABLE
	...
	set GigAFN(ucMode)  $DISABLE
	set GigAFN(ucRestart)  $::DISABLE
 	set GigAFN(ucEnableCCode)  $::DISABLE
 	LIBCMD HTSetStructure $::GIG_STRUC_AUTO_FIBER_NEGOTIATE 0 0 0 GigAFN 0 $H $S $P

	struct_new GigAltTX GIGAltTransmit
	set GigAltTX(ucEnableSS1)  $::DISABLE
	set GigAltTX(ucEnableSS2)  $::DISABLE
	set GigAltTX(ucEnableBG1)  $::DISABLE
	set GigAltTX(ucEnableBG2)  $::DISABLE	
 	set GigAltTX(ucEnableHoldoff)  $::ENABLE
   ...
   #///////////////////////////////set_atm//////////////////////////////////
	...
	AT-9015 {
		struct_new LineParams ATMDS1E1LineParams
    		set LineParams(ucFramingMode)  $::ATM_DS 1_CELL_FRAMING
    		...
    		set LineParams(ucLineBuildout)  $::ATM_DS1_0_TO_133_BUILDOUT
    		set LineParams(ucLineCoding)  $::ATM_DS1_B8ZS_ENCODING
    		set LineParams(ucLineFraming)  $::ATM_DS1_ESF_LINE_FRAMING
	...
	AT-9020 {
		struct_new LineParams ATMDS1E1LineParams
  		set LineParams(ucFramingMode)  $::ATM_E1_CELL_FRAMING
    		set LineParams(ucTxClockSource)  $::ATM_INTERNAL_CLOCK
    		set LineParams(ucCellScrambling)  $::TRUE
    		set LineParams(ucHecCoset)  $::TRUE
    		set LineParams(ucRxErroredCells)  $::ATM_CORRECT_ERRORED_CELLS
    		set LineParams(ucLoopBackEnable)  $::ATM_LOOPBACK_DISABLED
    		set LineParams(ucLineBuildout)  $::ATM_E1_BUILDOUT
    		set LineParams(ucLineCoding)  $::ATM_E1_HDB3_ENCODING
	...
	AT-9025 {
		struct_new LineParams ATMLineParams
  		set LineParams(ucFramingMode)  $::ATM_25_FRAMING
  		set LineParams(ucTxClockSource)  $::ATM_INTERNAL_CLOCK
  		set LineParams(ucCellScrambling)  $::TRUE
  		set LineParams(ucHecCoset)  $::TRUE
  		set LineParams(ucRxErroredCells)  $::ATM_CORRECT_ERRORED_CELLS
	...
	AT-9034 {
		...
		set LineParams(ucFramingMode)  $::ATM_E3_CELL_FRAMING
 		set LineParams(ucTxClockSource)   $::ATM_INTERNAL_CLOCK
 		set LineParams(ucCellScrambling)  $::TRUE
 		set LineParams(ucHecCoset)  $::TRUE
 		set LineParams(ucRxErroredCells)  $::ATM_CORRECT_ERRORED_CELLS
 		set LineParams(ucLoopBackEnable)  $::ATM_LOOPBACK_DISABLED
 		set LineParams(ucLineBuildout)  $::ATM_DS3_SHORT_BUILDOUT
 		for {set i 0} {$i < 4} {incr i} {
		  	set LineParams(ucIdleCellHeader.$i) 0
		}
	...
	AT-9622 {
		struct_new LineParams ATMLineParams
		set LineParams(ucFramingMode)  $::ATM_OC12_FRAMING
  		set LineParams(ucTxClockSource)  $::ATM_INTERNAL_CLOCK
  		set LineParams(ucCellScrambling)  $::TRUE
  		set LineParams(ucHecCoset)  $::TRUE
  		set LineParams(ucRxErroredCells)  $::ATM_CORRECT_ERRORED_CELLS
