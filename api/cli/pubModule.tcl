...
catch {package require http} err

#**********************************generic api*************************************

catch {package require http} err
catch {package require base64} err
...
namespace eval util {
	...
	proc chkConnect { url } {
	    #this proc must require activetcl 8.6 or above due to a bug for zlib in 8.5 version.
	    set username "admin"
		set password ""
	    
		set autheader [list "Authorization" [concat "Basic" [base64::encode $username:$password]]]
		set tok [http::geturl $url -headers $autheader]
		set respcode [http::ncode $tok]
		
		if { $respcode != 200 } {
		    puts "$url: Connection Fail"
			return -1
		} else {
			puts "$url: Connection successful"
			return 1
		}
		http::cleanup $tok
	}
	...
	proc setToFactoryDefault { dut } {
		...
		set ret [catch {eval $cmd} err]
		if { $ret } {puts "$err: setToFactoryDefault on $dut Failed"}
	}
