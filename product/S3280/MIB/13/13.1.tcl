...
##################Arp inspection introduction###########################
#Notes:
#The target of following test cases  is  test sender and/or targert protocol address is 0.0.0.0 in arp request packet
...
-sendProtAdd $errIp1 -targetHardAdd $unkonwMac -targetProtAdd $targIp   -dbgprt 1
...
if { $getCaptured == 0 } {
    passed  "Arp Inspection" "Arp Inspection  error sender protocol ip address 0.0.0.0  in arp Request packets test succeed"
} else { 
    failed  "Arp Inspection" "Arp Inspection  error sender protocol ip address 0.0.0.0  in arp Request packets test"
}


#.2 unexpected target protocol address 0.0.0.0  in arp request packets  test 
...
#.3 unexpected sender and target  protocol address 0.0.0.0  in arp request packets  test 

config_frame  -alias ixiap1 -srcmac $srcMac  -dstmac $destBroMac -frametype ethernetii -protocol arp -operation arpRequest -sendHardAdd  $srcMac  \
-sendProtAdd $errIp1 -targetHardAdd $unkonwMac -targetProtAdd $errIp1   -dbgprt 1
config_stream -alias ixiap1 -ratemode fps -fpsrate $::ixiafpsrate
clear_stat -alias allport
start_capture -alias ixiap2
send_traffic -alias ixiap1 -actiontype start -time 2
...
puts "get captured $getCaptured"
if { $getCaptured == 0 } {
    passed "Arp Inspection" "arp inspection  error sender and target protocol ip address 0.0.0.0  in arp Request packets test succeed"
} else { 
    failed  "Arp Inspection" "arp inspection  error sender and target protocol ip address 0.0.0.0  in arp Request packets test"
}
