...
#Notes:
#The target of following test cases  is  test ARP Inspection static table in vlan 2  worked well or not.
...
arpInspec::createStaticTable $dut1  $port1 2  $srcMac $sendIp
arpInspec::createStaticTable $dut1  $port2 2  $srcMac $sendIp


set portlist  [port2Portlist "$port1 $port2"]
vlan::vlanadd 2  $portlist  $dut1

vlan::porttypeSet $port1 cport $dut1
vlan::porttypeSet $port2 cport $dut1
...
if { $getCaptured != 0 } {
    passed  "Arp Inspection" "Arp Request packets that matched static table  from two direction  under vlan 2 test succeed"
} else { 
    failed  "Arp Inspection" "Arp Request packets that matched static table from two direction  under vlan 2 test  failed"
}

#.2  Arp Reply packets that Matched the arp inspection static table  test 

config_frame  -alias ixiap1 -srcmac $srcMac  -dstmac $destMac -vlanmode  singlevlan -vlanid  2  -frametype $ethtype -protocol arp -operation arpReply -sendHardAdd  $srcMac \
...
puts "get captured $getCaptured"
if { $getCaptured != 0 } {
    passed  "Arp Inspection" "Arp Reply packets that matched static table  from two direction under vlan 2 test succeed"
} else { 
    failed  "Arp Inspection" "Arp Reply packets that matched static table from two direction under vlan 2 test  failed"
}
