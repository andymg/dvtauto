...
##################Arp inspection introduction###########################
#Notes:
#The target of following test cases  is  test  targert protocol address is 0.0.0.0  and 255.255.255.255 in arp reply packet
...
#.1 unexpected target protocol address 0.0.0.0  in arp reply packets  test 

config_frame  -alias ixiap1 -srcmac $srcMac  -dstmac $destBroMac -frametype $ethtype -protocol arp -operation arpReply -sendHardAdd  $srcMac  \
-sendProtAdd $sendIp -targetHardAdd $unkonwMac -targetProtAdd $errIp1   -dbgprt 1
...
if { $getCaptured == 0 } {
    passed "Arp Inspection" "Arp Inspection  error target protocol ip address 0.0.0.0  in arp Reply packets test succeed"
} else { 
    failed  "Arp Inspection" "Arp Inspection  error target protocol ip address 0.0.0.0  in arp Reply packets test failed"
}


#.2 unexpected target protocol address 255.255.255.255  in arp reply packets  test 
...
puts "get captured $getCaptured"
if { $getCaptured == 0 } {
    passed "Arp Inspection" "Arp Inspection  error target protocol ip address 255.255.255.2555  in arp Reply packets test succeed"
} else { 
    failed  "Arp Inspection" "Arp Inspection  error target protocol ip address 255.255.255.255  in arp Reply packets test failed"
}
