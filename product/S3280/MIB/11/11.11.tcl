...
#Notes:
#The target of following test cases is to verify the dut will not add mld v1 multicast address 
#which is not in the configuration and to check the traffic for this multicast address.
...
# ################################################################################################
# ##################################mldv1 illeagal multicast addr test############################
mvrenable false
mvrenable true
config_frame -alias ixiap2 \
			 -framesize 200 \
...
set get_capture_p2 [check_capture -alias ixiap2 -dstmac "33 33 00 00 00 0B" -srcmac "00 00 00 00 00 01"]
set get_capture_p3 [check_capture -alias ixiap3 -dstmac "33 33 00 00 00 0B" -srcmac "00 00 00 00 00 01"]
if {[llength $group_entry==0] && $get_capture_p2==$get_capture_p3} {
	passed "mldv1 illeagal multicast addr test" "passed"
} else {
	failed "mldv1 illeagal multicast addr test" "failed"
}
