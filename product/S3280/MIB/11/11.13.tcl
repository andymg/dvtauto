...
#Notes: the target of following test cases is check the statistics for mld query packets received.
...
#################################################################################
##############mvr statistics check mld query received############################
clear_mvr_stat
...
if {$statis_entry($vid)==$ixiap1tx} {
	passed "mld query received statistics" "passed"
} else {
	failed "mld query received staitsitcs" "failed"
}