...
#Note : this test case is to test destination port can not send and receive traffice normally
...
#1. test mirroring destination port can not receive traffic
...
if { $getCaptured == 0 } {
    passed "Mirroring" "Mirroring destination port can not receive traffic test succeed"
} else { 
    failed  "Mirroring" "Mirroring destination port can not receive traffic  test failed"
}
...
#2. test mirroring destination port can not send  traffic

mirror::setDestPort $dut1 $port1
mirror::setSourcePort $dut1 $port2 disable
...
puts "get captured $getCaptured"
if { $getCaptured == 0 } {
    passed "Mirroring" "Mirroring destination port can not send traffic test succeed"
} else { 
    failed  "Mirroring" "Mirroring destination port can not send traffic test  failed"
}



#3. test mirroring destination port could receive STP packets 

mirror::setDestPort $dut1 $port1
set srcMac [getPortMacaddr $dut1 $port1]
set destMac "01 80 c2 00 00 00"

start_capture -alias ixiap1
after 10000
stop_capture -alias ixiap1 
get_stat -alias ixiap1 -rxframe ixiap1rx
puts "ixiap1_rx_frame: $ixiap1rx"
set getCaptured [check_capture -alias ixiap1 -srcmac $srcMac -dstmac $destMac  ]
puts "get captured $getCaptured"
if { $getCaptured != 0 } {
    passed "Mirroring" "Mirroring destination port coulde receive STP packets    test succeed"
} else { 
    failed  "Mirroring" "Mirroring destination port coulde receive STP packets   test failed"
}
