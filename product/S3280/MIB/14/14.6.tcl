...
#Notes:Ip Source Guard change dynamic binding table to static table test 
...
puts "get captured $b"
if { $a == 0 && $b != 0 } {
	puts "DUT forwards $a IP packets when Ip Source Guard static and dynamic table are empty"
	puts "DUT forwards $b IP packets when IP packets meatched the static table"
    passed  "Ip Source Guard" "Ip Source Guard change  dynamic binding table  to static table test succeed"
} else { 
	puts "DUT forwards $a IP packets when Ip Source Guard static and dynamic table are empty"
	puts "DUT forwards $b IP packets when IP packets meatched the static table"
    failed  "Ip Source Guard" "Ip Source Guard change  dynamic binding table  to static table test  failed"
}
