...
#Note :The target of this test case is to test dhcp request packets (DHCP inform ) from untrust port to trust port;
...
puts "IXIA $ixiap1 connect to dut untrust port $port1 and send DHCP inform packets from this port "
...
if { $staixiap2 == 0 && $staixiap3 != 0  } {
    passed "DHCP Snooping" "Send DHCP INFORM packets from untrust port to trust port test  succeed"
} else { 
    failed  "DHCP Snooping" "Send DHCP INFORM packets from untrust port to trust port test  failed"
}
