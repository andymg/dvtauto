...
#Note :The target of this test case is to test dhcp request packets (DHCP decline  and release ) from trust port to trust or untrust port;
...
# 1. sending  DHCP decline packets from trust port test 
...
puts "IXIA $ixiap1 connect to dut trust port $port1 and send DHCP decline packets from this port "
...
if { $staixiap2 == 0 && $staixiap3 != 0  } {
    passed "DHCP Snooping" "Send DHCP DECLINE packets from trust port to trust or untrust port test  succeed"
} else { 
    failed  "DHCP Snooping" "Send DHCP DECLINE packets from trust port to trust or untrust port test  failed"
}


#2. sending  DHCP release packets from trust port test 
...
get_stat -alias ixiap2 -rxframe ixiap2rx
get_stat -alias ixiap3 -rxframe ixiap3rx
puts "IXIA $ixiap1 connect to dut trust port $port1 and send DHCP release packets from this port "
set staixiap2 [check_capture -alias ixiap2 -srcmac $srcmac  ]
...
if { $staixiap2 == 0 && $staixiap3 != 0  } {
    passed "DHCP Snooping" "Send DHCP RELEASE packets from trust port to trust or untrust  port test  succeed"
} else { 
    failed  "DHCP Snooping" "Send DHCP RELEASE packets from trust port to trust or untrust  port test  failed"
}
