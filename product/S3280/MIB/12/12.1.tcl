...
#Note :The target of this test case is to test dhcp request packets (DHCP discover, and request) from untrust port to trust port;
...
puts "IXIA $ixiap1 connect to dut untrust port $port1 and send DHCP discover packets from this port "
...
#2. sending  DHCP request packets from untrust port test 
...
get_stat -alias ixiap2 -rxframe ixiap2rx
get_stat -alias ixiap3 -rxframe ixiap3rx
puts "IXIA $ixiap1 connect to dut untrust port $port1 and send DHCP request packets from this port "
set staixiap2 [check_capture -alias ixiap2 -srcmac $srcmac  ]
...
if { $staixiap2 == 0 && $staixiap3 != 0  } {
    passed "DHCP Snooping" "Send DHCP REQUEST packets from untrust port to trust port test  succeed"
} else { 
    failed  "DHCP Snooping" "Send DHCP REQUEST packets from untrust port to trust port test  failed"
}
