...
#Notes:
#The target of this test case is to test Port VLAN Mode function
...
#This mode is normally used for ports connected to VLAN aware switches. Tx tag should be set to Untag_pvid when this mode is used.
#If Specific (the default value) is selected, a Port VLAN ID can be configured (see below).
#Untagged frames received on the port are classified to the Port VLAN ID. 
#If VLAN awareness is disabled, all frames received on the port are classified to the Port VLAN ID. 
#If the classified VLAN ID of a frame transmitted on the port is different from the Port VLAN ID, 
