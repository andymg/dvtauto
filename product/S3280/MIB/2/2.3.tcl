...
#Notes:
#The target of this test case is to test if c-s-port can work well or not?
...
proc sendReceive { vid tpid srcmac rate name desc {mark default} } {
    ...
    switch mark {
       1    {
                if { $getCaptured ==0 } { 
                    if { $tpid == 9100 || $tpid == 9200 } {
                       passed $name $desc
                    } else {
                       failed $name $desc
                    }
                } else {
                    if { $tpid == 8100 || $tpid == 88a8 } {
                       passed $name $desc
                    } else {
                       failed $name $desc
                    }
...
set desc "send untag packets to check receive packet type when Port Type is c-s-port"
...
set ethtypeval 9100
vlan::etherTypeSet $ethtypeval

foreach vid { 0 1 } {
    foreach id {8100 88a8 9100 9200} {
        set name "VLAN ID is $vid, TPID is $id, EtherType for custom s-port is $ethtypeval"
        set desc "send Tag $vid to check receive VLAN Tag when Port Type is c-s-port"
        set mark 1 
...
foreach id {8100 88a8 9100 9200} {
    set name "VLAN ID is $vid, TPID is $id, EtherType for custom s-port is $ethtypeval, it doesn't add any port list"
    set desc "send Tag $vid to check receive VLAN Tag when Port Type is c-s-port"
    set mark 2
    sendReceive $vid $id $::ixiaMac1 $::ixiarate $name $desc $mark
}
