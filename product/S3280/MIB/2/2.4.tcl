...
#Notes:
#The target of this test case is to test if unaware-port can work well or not?

#precondition: all configurre should be default value except below mention.

#steps:
#1.when vlan=1 by default, send priority-vlan with different TPID,CFI and VID,to check its result.
#2.when vlan=1 by default, send singlevlan with different TPID,CFI and VID, to check its result.
#3.when vlan=1 by default, send doublevlan with different TPID,CFI and VID, to check its result.
#4.when vlan=other VID by default, to retest step1-3.
...
proc sendReceive { vid tpid srcmac rate name desc } {
...
set desc "send untag packets to check receive packet type when Port Type is unaware-port"
...
foreach vid { 0 1 } {
    foreach id {8100 88a8 9100 9200} {
        ...
        sendReceive $vid $id $::ixiaMac1 $::ixiarate $name $desc
	}
}
