...
def xmasscan(macsrc,ipsrc,ipdst,mgmtnic,trfcnic,num):
	AttackTest = AttackCases(ipdst)
	try:
		print "First to Test Management Port Attack"
		send(IP(dst=ipdst)/TCP(dport=666,flags="FPU"),count=num,iface=mgmtnic)
		...
		send(IP(dst=ipdst)/TCP(dport=666,flags="FPU"),count=num,iface=trfcnic)
		conf.route.resync()
		AttackTest.ChkDutSts()
	except:
		print "FAIL"
...
if __name__=="__main__":
	...
	print "===========Start Testing xmasscan============="
	xmasscan(macsrc,ipsrc,ipdst,mgmtnic,trfcnic,num)
	print "=========xmasscan testing finished========="
	print "PASS"
